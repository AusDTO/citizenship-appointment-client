...
request.onload = () => {
  if (request.status >= 200 && request.status < 300) {
    ...
    let foo = calendar(availableDates, document.getElementById('calendar').getAttribute('data-today'));
    calendarRenderer(foo);
    if(window.location.hash) {
      router(foo, true)();
    }
    window.addEventListener("hashchange", (e) => {
      router(foo, false)(); }, null);
  } else {
...
request.onerror = () => {
  dom.renderSystemError('.main-container');
};
...
function expireSession() {
    window.location = "/session_timeout?page_name='Session Timeout'";
}
...
function sessionExpiryNotification() {
  ...
  document.getElementById('extend_session').addEventListener('click', () => {
