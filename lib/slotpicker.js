...
class SlotPicker {
  ...
  bindEvents() {
    ...
    $('.BookingCalendar-dateLink').click(function() {
      ...
      self.availableTimesForDate('2015-12-15', () => {
        document.querySelector(`tr.CalRow-slots[data-week='${week}'] ul`).innerHTML = self.settings.bookingData['2015-12-15'].times;
      });
    });
  }
  ...
  // HELPERS
  availableTimesForDate(date, callback) {
    if (!this.settings.bookingData[date].times) {
      let request = new XMLHttpRequest();
      request.open('GET', 'get_available_times.json');
      request.onload = () => {
        if (request.status === 200) {
          let availableTimes = JSON.parse(request.responseText).times;
          this.settings.bookingData[date].times = availableTimes;
          callback();
        } else {
          // FIXME(Emily)
        }
