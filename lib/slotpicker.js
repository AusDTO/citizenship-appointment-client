...
class SlotPicker {
  constructor(bookingData) {
    ...
    for(let date of Object.keys(bookingData)) {
      ...
      bookingData[date].availableTimes = new Promise( function (resolve, reject) {
        setTimeout(()=>{let request = new XMLHttpRequest();
  ...
  selectedTimeListener() {
    ...
    $('.BookingCalendar-timeLink').click(function() {
      ...
      let [hours, minutes] = self.settings.selectedTime.split(":");
      data['selected_appointment'] = moment.utc(self.settings.selectedDate).add({hours, minutes}).format('YYYY-MM-DDTHH:mm:SS');
