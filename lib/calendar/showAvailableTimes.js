...
module.exports = (dateString, availableDates, todayDate) => {
  if (!availableDates[dateString]) { return; }
  let handlePromise = (availableTimes) => {
    ...
    var mappedAT = availableTimes.map((time) => {
        return {
          ...
          ampm: time.amPm(),
          aria_display_time: time.ariaDisplayDateTime()
        };
      })

    var calendarDate = document.querySelector(`a.date-link.date-${dateString}`);
    var startIndex = calendarDate.tabIndex+1;
    ...
    document.querySelector(`.appointment-content.date-${dateString}`).innerHTML = renderedTimesHtml;

    if (window.location.hash) {
      let [hashType, hashValue] = location.hash.split("/");
      if (hashType === '#time') {
        let className = datetime(hashValue).toDateTimeClassString();
        dom.addClass(`.appointment-list-item.datetime-${className}`, 'is-active');
        dom.addClass(`.appointment-link.datetime-${className}`, 'is-active');
  ...
  let handlePromiseError = (err) => {
    if(err.status === 401) {
      ...
      err.status ? window.location = "error" : dom.renderSystemError('.main-container');
    }
  };
  ...
  dom.toggleHideOn('.appointment-section');
  dom.toggleShowOn(`.appointment-section.date-${dateString}`);
