	  ...
	  _createClass(SlotPicker, [{
	    ...
	    value: function selectedTimeListener() {
	      ...
	      $('.BookingCalendar-timeLink').click(function () {
	        ...
	        var _self$settings$select = self.settings.selectedTime.split(":");
	
	        var _self$settings$select2 = _slicedToArray(_self$settings$select, 2);
	
	        var hours = _self$settings$select2[0];
	        var minutes = _self$settings$select2[1];
	
	        data['selected_appointment'] = moment.utc(self.settings.selectedDate).add({ hours: hours, minutes: minutes }).format('YYYY-MM-DDTHH:mm:SS');
